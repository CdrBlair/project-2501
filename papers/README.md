# Papers

This directory contains academic papers and companion analyses that form the empirical and theoretical evidence base for the AI-Augmented SDLC Framework. The collection spans 1985 to 2025, deliberately selecting sources for their empirical rigour and theoretical coherence rather than industry influence.

---

## Index

### Primary Sources (PDFs)

Academic papers providing foundational research and evidence.

| Document | Description |
|----------|-------------|
| [naur-1985-programming.pdf](./naur-1985-programming.pdf) | Naur, P. (1985). "Programming as Theory Building" — Foundational paper arguing programming is cognitive theory construction |
| [dynamic-theory-organizational-knowledge-creation.pdf](./dynamic-theory-organizational-knowledge-creation.pdf) | Nonaka, I. (1994). "A Dynamic Theory of Organizational Knowledge Creation" — SECI model for knowledge conversion |
| [team-tacit-knowledge-measure.pdf](./team-tacit-knowledge-measure.pdf) | Ryan & O'Connor (2013). Team tacit knowledge measurement instrument with validated scales |
| [acquiring-sharing-tacit-knowledge-software.pdf](./acquiring-sharing-tacit-knowledge-software.pdf) | Research on mechanisms for acquiring and sharing tacit knowledge in software teams |
| [devtime-preprint-tse19.pdf](./devtime-preprint-tse19.pdf) | Meyer et al. (2019). "What Constitutes a Good Day?" — Large-scale study of developer productivity factors |
| [developer-thriving.pdf](./developer-thriving.pdf) | Hicks et al. (2024). "Developer Thriving" — Four sociocognitive factors for resilient productivity (N=1,282) |
| [hicks-hevesi-cumulative-culture-theory.pdf](./hicks-hevesi-cumulative-culture-theory.pdf) | Hicks & Hevesi (2024). Cumulative culture theory applied to software development |
| [context-engineering-v2.pdf](./context-engineering-v2.pdf) | Hua et al. (2025). "Context Engineering 2.0" — Survey of context engineering for LLMs/agents |

### Companion Analyses

Detailed analytical documents that extract key insights from primary sources and relate them to the framework.

| Document | Description |
|----------|-------------|
| [naur-1985-programming-companion.md](./naur-1985-programming-companion.md) | Comprehensive analysis of Naur's theory-building paper including bibliographic data, methodology assessment, and framework integration |
| [wegner-1986-transactive-memory-companion.md](./wegner-1986-transactive-memory-companion.md) | Analysis of Wegner's transactive memory theory and its application to software teams |
| [team-tacit-knowledge-measure-companion.md](./team-tacit-knowledge-measure-companion.md) | Analysis of the team tacit knowledge measurement instrument and validation approach |
| [meyer-good-day-companion.md](./meyer-good-day-companion.md) | Analysis of Meyer et al.'s research on developer good days and productivity factors |
| [developer-thriving-companion.md](./developer-thriving-companion.md) | Analysis of the Developer Thriving framework and its four sociocognitive factors |
| [context-engineering-v2-companion.md](./context-engineering-v2-companion.md) | Analysis of Context Engineering 2.0 survey, tracing evolution from early NLP to LLM agents |
| [illusion-of-thinking-companion.md](./illusion-of-thinking-companion.md) | Analysis of AI reasoning limitations and implications for human-AI collaboration |
| [productivity-quality-companion.md](./productivity-quality-companion.md) | Analysis of productivity and quality research with measurement framework implications |

### Reference Documents

Curated reference materials and integration analyses.

| Document | Description |
|----------|-------------|
| [reference-library-quality-annotated.md](./reference-library-quality-annotated.md) | Annotated bibliography with quality tier assessments for all reference sources |
| [reference-library-standards-update.md](./reference-library-standards-update.md) | Analysis of standards evolution and current status (ISO/IEC/IEEE 12207, etc.) |
| [naur-wegner-integration-research.md](./naur-wegner-integration-research.md) | Integration analysis connecting Naur's theory-building with Wegner's transactive memory |
| [tacit-knowledge-software-failure.md](./tacit-knowledge-software-failure.md) | Analysis of how tacit knowledge loss contributes to software project failures |

---

## Summary

This papers collection represents a curated evidence base selected according to explicit criteria prioritising empirical validity over industry influence. The selection acknowledges that **influence and validity are orthogonal concerns**—concepts can be widely adopted yet empirically unsupported, or rigorously validated yet underutilised in practice.

### Selection Criteria

Each source meets multiple criteria:

**Essential Criteria:**
- Theoretical coherence with clear conceptual frameworks
- Empirical grounding through data or validated evidence synthesis
- Methodological transparency enabling replication
- Generalisability potential beyond narrow contexts
- Distinct conceptual contribution

**Quality Indicators:**
- Peer review or equivalent validation (7/10 core sources are peer-reviewed)
- Large sample sizes where applicable (N>2,000 in multiple studies)
- Triangulation across methods
- Longitudinal or cross-sectional depth

### Temporal Coverage

The collection deliberately spans four decades:

| Era | Sources | Contribution |
|-----|---------|--------------|
| **1985-1990** | Naur (1985), Wegner (1986) | Foundational theory on individual cognition and team memory |
| **1994** | Nonaka (1994) | Knowledge creation and conversion models |
| **2013-2019** | Ryan & O'Connor (2013), Meyer et al. (2019) | Empirical measurement instruments and validation |
| **2022-2024** | Storey et al. (2022), Hicks et al. (2024) | Developer experience frameworks and thriving factors |
| **2025** | Context Engineering 2.0, Illusion of Thinking | AI era considerations and limitations |

### Companion Document Structure

Each companion analysis follows a consistent structure:

1. **Bibliographic Information** — Full citation, publication venue, quality tier assessment
2. **Source Classification** — Type of work, nature of contribution
3. **Methodology Assessment** — Study design, sample characteristics, analytical approach
4. **Key Findings** — Central claims and evidence
5. **Framework Integration** — Connection to other framework concepts
6. **Limitations and Validation Status** — What's validated, what requires caution

### Excluded Sources

The collection deliberately excludes influential but empirically unsupported concepts:
- "Move fast and break things" (high influence, likely negative outcomes)
- "10x programmer" myth (widely referenced, empirically contested)
- "Technical debt" metaphor (useful framing, rarely measured rigorously)

These exclusions reflect the principle of preferring **falsifiable knowledge** over **appealing narratives**.

### Usage

These papers and analyses serve as the empirical foundation for framework claims. When the framework asserts something about tacit knowledge, developer productivity, or AI limitations, the supporting evidence can be traced back to specific sources with documented methodology and quality assessment.
